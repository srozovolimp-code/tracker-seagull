  ...
  return a("div",{className:"team-table-root",children:[
    n.length?a("div",{className:"table-wrap",children:[
      ...
      groups.map((group,index)=>{const collapsed=collapsedTeams.includes(group.team);return a("section",{className:_("task-group",collapsed&&"collapsed"),children:[
        ...
        !collapsed&&a("div",{className:"team-branch-content",children:[
          group.tasks.map(task=>a("div",{className:"task-table task-row",children:[
            a("button",{className:_("task-title-cell",task.parentId&&"nested"),onClick:()=>s(task),children:[e("span",{className:"task-drag-mark",children:"⋮⋮"}),e("span",{children:task.title})]}),
...
/* Team branch collapse and members popup */
...
.team-collapse-button{width:27px;height:27px;border:0;border-radius:7px;background:transparent;color:#687587;display:grid;place-items:center;cursor:pointer}
.team-collapse-button:hover{background:#e9eef5;color:#3f5f8c}
.team-collapse-button span{display:block;font-size:20px;line-height:1;transform:rotate(0deg);transition:transform .16s ease}
.team-collapse-button.collapsed span{transform:rotate(-90deg)}
...
.task-group.collapsed .team-group-header{border-bottom:0}
.team-branch-content{overflow:hidden}
.task-drag-mark{color:#a6afbb;letter-spacing:-3px;font-size:13px}
.team-members-backdrop{position:fixed;inset:0;z-index:130;background:rgba(19,28,42,.45);backdrop-filter:blur(6px);display:grid;place-items:center;padding:20px}
.team-members-dialog{width:min(720px,100%);max-height:calc(100vh - 40px);display:flex;flex-direction:column;border-radius:19px;background:#fff;overflow:hidden;box-shadow:0 30px 90px rgba(16,27,45,.34)}
.team-members-head{min-height:92px;padding:17px 19px 14px;border-bottom:1px solid #e8edf3;display:flex;align-items:flex-start;justify-content:space-between;gap:20px}
.team-members-head span{display:block;color:#8190a4;text-transform:uppercase;letter-spacing:.09em;font-size:9px;font-weight:800}
.team-members-head h2{margin:6px 0 3px;font-size:22px;letter-spacing:-.6px}
.team-members-head p{margin:0;color:#8490a0;font-size:9px}
.team-members-head>button{width:35px;height:35px;border:0;border-radius:10px;background:#f1f4f8;color:#647186;display:grid;place-items:center}
.team-members-list{padding:9px;overflow-y:auto}
...
.team-members-empty{min-height:310px;padding:35px;display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center;color:#8792a1}
.team-members-empty h3{margin:13px 0 5px;color:#394557;font-size:14px}
.team-members-empty p{margin:0;max-width:350px;font-size:9px;line-height:1.5}
.team-members-footer{padding:10px 13px;border-top:1px solid #e8edf3;display:flex;justify-content:flex-end;background:#fafbfd}
.team-members-footer button{min-height:35px;padding:0 13px;border:0;border-radius:9px;background:#e3ecf9;color:#3d6398;font-size:10px;font-weight:750}
@media(max-width:680px){.team-members-backdrop{padding:0;place-items:end center}.team-members-dialog{width:100%;max-height:88vh;border-radius:19px 19px 0 0}.team-member-card{grid-template-columns:38px minmax(0,1fr)}.team-member-avatar{width:38px;height:38px}.team-member-actions{grid-column:2;flex-wrap:wrap}.team-member-actions a{min-height:29px}.team-group-header{position:sticky;left:0}}
`;
